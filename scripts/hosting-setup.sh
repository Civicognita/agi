...
echo "[...] Caddy: ensuring host caddy.service is stopped + disabled..."
# Story #100 cutover: production runs the rootless containerized agi-caddy
# user service (so it can reach project containers via the `aionima` podman
# network). The host-mode caddy.service still gets installed by the apt
# package, but it must NOT be the active reverse-proxy on :80/:443 — its
# Caddyfile points to `host.containers.internal:3100`, a hostname that only
# resolves inside containers. Leaving host caddy enabled causes a port
# collision with agi-caddy on next start AND serves 502s if it wins the race.
systemctl stop caddy 2>/dev/null || true
systemctl disable caddy 2>/dev/null || true
echo "[OK] Host caddy.service disabled — agi-caddy (user, rootless container) is the active proxy"
