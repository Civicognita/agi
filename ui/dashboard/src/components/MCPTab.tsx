...
function McpServerCard({
    ...
    try {
      ...
      if (callArgs.trim().length > 0) {
        try {
          ...
          setCallResult(`Argument parse error: arguments must be valid JSON`);
          setCalling(false);
          return;
        }
      }
      ...
      setCallResult(body.ok ? JSON.stringify(body.result, null, 2) : `Error: ${body.error ?? "unknown"}`);
    } catch (err) {
  ...
  return (
    <div className="rounded border border-border bg-surface1/50" data-testid={`mcp-card-${server.id}`}>
      ...
      {expanded && (
        <div className="border-t border-border px-3 py-3 space-y-3" data-testid={`mcp-card-body-${server.id}`}>
          {!isConnected && (
          ...
          {isConnected && (
            <>
              <div className="flex gap-1 text-[11px]">
                {(["tools", "prompts", "resources"] as const).map((t) => (
                  <button
                    ...
                    data-testid={`mcp-card-tab-${t}-${server.id}`}
                  >
                    {t.charAt(0).toUpperCase() + t.slice(1)}
                    {t === "tools" && tools !== null && ` (${String(tools.length)})`}
                    {t === "prompts" && prompts !== null && ` (${String(prompts.length)})`}
                    {t === "resources" && resources !== null && ` (${String(resources.length)})`}
                  </button>
                ))}
                <button
              ...
              {browseError && <div className="text-[11px] text-red">{browseError}</div>}

              {activeTab === "tools" && tools !== null && (
                <ul className="space-y-1.5">
                  {tools.length === 0 && <li className="text-[11px] text-muted-foreground italic">No tools.</li>}
                  {tools.map((t) => {
                      ...
                      <li key={t.name} className="text-[11px]">
                        ...
                        {isSelected && (
                          <div className="ml-2 mt-1.5 p-2 rounded border border-border bg-background space-y-2">
                            ...
                            {callResult !== null && (
                              <pre className="text-[10px] font-mono p-1.5 rounded bg-surface1 overflow-x-auto whitespace-pre-wrap max-h-64 overflow-y-auto">{callResult}</pre>
                            )}
                          </div>
                        )}
                      </li>
              ...
              {activeTab === "prompts" && prompts !== null && (
                <ul className="space-y-1">
                  {prompts.length === 0 && <li className="text-[11px] text-muted-foreground italic">No prompts.</li>}
                  {prompts.map((p) => (
              ...
              {activeTab === "resources" && resources !== null && (
                <ul className="space-y-1.5">
                  {resources.length === 0 && <li className="text-[11px] text-muted-foreground italic">No resources.</li>}
                  {resources.map((r) => {
                    ...
                    return (
                      <li key={r.uri} className="text-[11px]">
                        ...
                        {isSelected && callResult !== null && (
                          <pre className="ml-2 mt-1 text-[10px] font-mono p-1.5 rounded bg-surface1 overflow-x-auto whitespace-pre-wrap max-h-64 overflow-y-auto">{callResult}</pre>
                        )}
                      </li>
                    );
                  })}
