...
export async function createGatewayRuntimeState(
  ...
  if (deps.marketplaceManager) {
    ...
    // -----------------------------------------------------------------------
    // Claude Max provider — connection status + OAuth flow (mirrors Cloudflare pattern)
    // -----------------------------------------------------------------------

    fastify.get("/api/providers/claude-max/status", async (request, reply) => {
      if (!isPrivateNetwork(getClientIp(request.raw))) return reply.code(403).send({ error: "Private network only" });
      ...
      try {
        ...
        const creds = JSON.parse(raw) as { claudeAiOauth?: { accessToken?: string; expiresAt?: number; subscriptionType?: string; rateLimitTier?: string; scopes?: string[] } };
        ...
        return reply.send({
          ...
          expiresInHours: oauth.expiresAt ? Math.max(0, Math.round((oauth.expiresAt - Date.now()) / 3600_000)) : 0,
          scopes: oauth.scopes ?? [],
        });
      } catch {
        return reply.send({ connected: false });
    ...
    fastify.post("/api/providers/claude-max/connect", async (request, reply) => {
      if (!isPrivateNetwork(getClientIp(request.raw))) return reply.code(403).send({ error: "Private network only" });
      // Spawn `claude login` which opens the browser for Anthropic OAuth.
      // Same pattern as cloudflared login — fire the process, return immediately,
      // let the dashboard poll /status until credentials appear.
      try {
        const { execFile } = await import("node:child_process");
        const child = execFile("claude", ["login"], { timeout: 120_000 });
        child.on("error", () => { /* ignore — user may not have claude installed */ });
        child.on("close", () => { /* done — credentials file written by claude */ });
        return reply.send({ ok: true, message: "Authentication started. Complete the login in the browser window that opened." });
      } catch (err) {
        return reply.code(500).send({ error: err instanceof Error ? err.message : String(err) });
      }
    });

    fastify.post("/api/providers/claude-max/disconnect", async (request, reply) => {
      if (!isPrivateNetwork(getClientIp(request.raw))) return reply.code(403).send({ error: "Private network only" });
      const credPath = join(homedir(), ".claude", ".credentials.json");
      try {
        if (existsSync(credPath)) {
          // Don't delete the file — other tools (Claude Code) use it.
          // Instead, clear just the claudeAiOauth section.
          const raw = readFileSync(credPath, "utf-8");
